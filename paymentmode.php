<?php 
include('./include/header.php')
?> 
<section class="paymentmode-section">
  <div class="container mt-4">
    <div class="row">
      <div class="col-lg-12 col-md-12 d-lg-none d-md-block">
        <div class="note-container">
            <h1>Important  Note!</h1>
            <p>Thank you for ordering from Newlands Pharmacy. We request you to confirm your payment by replying on order confirmation email. Apologies for the inconvenience, we are setting up a new automated payment system.We request your patience. We would update you soon, so your payments can be done at a click!</p>
        </div>
      </div>
      <div class="col-lg-12 col-sm-12 payment-main">
        <!-- Paypal -->
        <div class="paymentmode-main">
          <div class="d-flex justify-content-between">
            <div class="icon-image d-flex align-items-center">
              <img src="./assets/images/paypal.png" alt="">
              <p class="icon-image-content">Paypal</p>
            </div>
            <div class="icon-content d-flex align-items-center">
              <button>Pay<span> <p class="icon-p">$159.31</p></span></button>
            </div>
          </div>
        </div>
        <!-- credit -->
        <div class="paymentmode-main">
          <div class="d-flex justify-content-between">
            <div class="icon-image d-flex align-items-center">
              <img src="./assets/images/credit-card.png" alt="">
              <p class="icon-image-content">Credit Card</p>
            </div>
            <div class="icon-content d-flex align-items-center">
              <button>Pay<span> <p class="icon-p">$159.31</p></span></button>
            </div>
          </div>
        </div>
        <!-- debit -->
        <div class="paymentmode-main">
          <div class="d-flex justify-content-between">
            <div class="icon-image d-flex align-items-center">
              <img src="./assets/images/debit.png" alt="">
              <p class="icon-image-content">Debit Card</p>
            </div>
            <div class="icon-content d-flex align-items-center">
            <button>Pay<span> <p class="icon-p">$159.31</p></span></button>
            </div>
          </div>
        </div>
        <!-- cash app -->
        <div class="paymentmode-main">
          <div class="d-flex justify-content-between">
            <div class="icon-image d-flex align-items-center">
              <img src="./assets/images/cash-app.png" alt="">
              <p class="icon-image-content">Cash App</p>
            </div>
            <div class="icon-content d-flex align-items-center">
            <p class="icon-p-con1">Extra 5% Off</p>

              <button>Pay<span> <p class="icon-p">$159</p></span></button>
            </div>
          </div>
        </div>
        <!-- zelle -->
        <div class="paymentmode-main">
          <div class="d-flex justify-content-between">
            <div class="icon-image d-flex align-items-center">
              <img src="./assets/images/Zelle-Symbol.png" alt="">
              <p class="icon-image-content">Zelle</p>
            </div>
            <div class="icon-content d-flex align-items-center">
            <p class="icon-p-con2">Extra 5% Off</p>

              <button>Pay<span> <p class="icon-p">$<span class="discount">159</span></p></span></button>
            </div>
          </div>
        </div>
        <!-- western union -->
        <div class="paymentmode-main">
          <div class="d-flex justify-content-between">
            <div class="icon-image d-flex align-items-center">
              <img src="./assets/images/western-union.png" alt="">
              <p class="icon-image-content">Western Union</p>
            </div>
            <div class="icon-content d-flex align-items-center">

              <button>Pay<span> <p class="icon-p">$159.31</p></span></button>
            </div>
          </div>
        </div>
        <!-- bitcoin -->
        <div class="paymentmode-main">
          <div class="d-flex justify-content-between">
            <div class="icon-image d-flex align-items-center">
              <img src="./assets/images/Bitcoin..png" alt="">
              <p class="icon-image-content">Bitcoin</p>
            </div>
            <div class="icon-content d-flex align-items-center">

              <button>Pay<span> <p class="icon-p">$159.31</p></span></button>
            </div>
          </div>
        </div>
        <!-- Wiretransfer -->
        <div class="paymentmode-main">
          <div class="d-flex justify-content-between">
            <div class="icon-image d-flex align-items-center">
              <img src="./assets/images/western-union.png" alt="">
              <p class="icon-image-content">Wiretransfer</p>
            </div>
            <div class="icon-content d-flex align-items-center">

              <button>Pay<span> <p class="icon-p">$159.31</p></span></button>
            </div>
          </div>
        </div>
        <!-- MoneyGram -->
        <div class="paymentmode-main">
          <div class="d-flex justify-content-between">
            <div class="icon-image d-flex align-items-center">
              <img src="./assets/images/MoneyGram.png" alt="">
              <p class="icon-image-content">MoneyGram</p>
            </div>
            <div class="icon-content d-flex align-items-center">

              <button>Pay<span> <p class="icon-p">$159.31</p></span></button>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
  </div>
</section> 
<?php 
include('./include/footer.php')
?>